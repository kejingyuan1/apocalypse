...
路径 `fortress/`，`core/` 逻辑 + `main.gd`/`hud.gd`：

- **架构方向正确**：逻辑在 `core/`（纯 `RefCounted`，无节点依赖，符合 ADR-002 可服务端化），`main.gd` 只做输入翻译+渲染，`hud.gd` 用暖橙。
...
| 11 | `room_defs.gd` 房间面积矩阵        | 指挥 4×4 / 防御 3×3 / 生产 3×2                                   | 同上（建造 §7 面积矩阵）                              | 与 Lv1=6 网格几何冲突 | ⚠️ 已调整：指挥 2×2 / 防御 1×1 / 生产 2×1，使初始 6×6 网格可建造；见 §七说明                                                         |
...
### Phase C 追加修复记录（2026-08-21 二次：窗口化可玩性抢修）

- **用户反馈**：窗口化运行后画面只有左上角一个小灰点，不像游戏；且按 2 无法放置防御塔。
- **根因**：
  1. **房间面积与初始网格几何冲突**：GDD 指定指挥 4×4、防御 3×3、生产 3×2，而 Lv1 网格仅 6×6；4×4 核心一放，剩余 L 形边界最宽只有 2 格，**任何 3×3 防御/3×2 生产都放不下**，导致玩家第 1 波完全无法造塔。
  2. **无相机居中/缩放**：6×6 网格只有 192×192 像素，在默认 1152×648 窗口里缩在左上角。
  3. **无网格背景**：大片灰色看不出 play area。
  4. **丧尸速度 `BASE_SPEED=20` 格/秒**：过快导致穿墙/跳过目标；被墙完全包围时 BFS 距离场失效，丧尸无法定位墙壁。
- **修复**：
  - `core/room_defs.gd`：房间尺寸改为可放进 6×6 网格 —— 指挥 2×2、防御 1×1（炮塔）、生产 2×1、城墙 1×1；保留原 HP 数值。
  - `main.gd`：2×2 核心居中放置；新增 `Camera2D` 动态缩放使网格填满窗口；新增暗色背景 `ColorRect`；新增 `_draw()` 网格线；精灵按房间格数缩放。
  - `core/zombie.gd`：`BASE_SPEED` 从 20.0 改为 **1.5 格/秒**（待后续平衡），避免跳墙。
  - `core/battle_sim.gd`：新增 BFS 不可达时的几何向核心推进兜底（`_move_toward_core_geometric`），确保被墙隔离时仍会攻击障碍。
  - `tools/validate.gd`：场景 B 城墙环按 2×2 核心重算（12 块墙）；新增场景 D「可赢性探针」：5 座防御塔守第 1 波，核心满血存活。
- **结果**：headless 验证通过，且证明游戏在合理布防下**可赢**。

### Phase C 视觉大修记录（2026-08-21 三次：从调试视图到游戏界面）

- **用户反馈**：窗口化画面像「冰冷的低质贴图」，不像游戏。
- **根因**：
  1. 继续使用原 `assets/art/*.png` 低清精灵，并按房间格数拉伸，导致画面模糊、比例怪异。
  2. 无整体视觉设计：无房间色块区分、无 HP 条、无悬停提示、无网格背景层次、HUD 只是两行文字。
- **修复**：
  - `core/room_defs.gd`：新增 `color(type)` 与 `symbol(type)`，为每类房间定义清晰色块与中文字符标识（城墙=石褐/墙、防御=炮塔蓝/防、生产=绿/产、指挥=暖橙/核）。
  - `main.gd`：
    - 重写渲染层，全部改用 `_draw()` 矢量绘制；移除拉伸精灵。
    - 新增棋盘格地面、粗细分级网格线、房间色块 + 高光/阴影边。
    - 城墙画砖缝；指挥核心画内部能量高光。
    - 受损房间与丧尸均显示 HP 条（绿/橙/红三段）。
    - 建造态悬停显示半透明预览 + 虚线框；悬停/放置防御塔时显示射程圈。
    - 丧尸绘制为带眼睛朝向的彩色圆点，并标注种类（走/跑/喷）。
    - `Camera2D` 正确居中（`ANCHOR_MODE_DRAG_CENTER` + `make_current`）并按窗口大小动态缩放，使网格始终填满画面。
  - `hud.gd`：
    - 顶部状态条居中，使用暖橙威胁色。
    - 底部新增半透明面板，左侧显示彩色资源（废料金黄 / 生物质绿），中间显示当前选中房间与造价，右侧显示操作键位。
